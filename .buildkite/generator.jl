...
if IS_PR
    ...
    COMPARE_AGAINST = readchomp(`git merge-base --fork-point $(TARGET_BRANCH) HEAD`)
...
if length(PROJECTS_ACCEPTED) > 20
    annotate("Too many projects", style="error")
    exit(1)
end
...
for PROJECT in PROJECTS_ACCEPTED
    ...
    STEPS = Any[]
    if !IS_PR
        push!(STEPS, jll_init_step(NAME, PROJECT, BB_HASH, PROJ_HASH, SKIP_BUILD))
    ...
    if !IS_PR
        push!(STEPS, wait_step())
        push!(STEPS, register_step(NAME, PROJECT, BB_HASH, PROJ_HASH, SKIP_BUILD))
    end
